...
Összefoglalás, jövő (közvetlenül a mérések után):

- almát a körtével
- nem a részletes összenhasonlítgatás volt a cél
- proof of concept
...
1. A legtöbb rendszer már úgy van fejlesztve, hogy van hely bőven. Miért kritikus ez egy
mockadatbázisnál? A laptopok is rendelkeznek ár több GB memóriával.

- egy lightweight megoldás megnöveli a lehetőségeket
- olyan új lehetőségek nyílnak meg, amelyeket eddig nem is asszociáltak adatbázisokhoz (dev mode, serverless stb.)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

2. A tranzakcióknál az írási státusznál feltételezi, hogy addig nem olvas senki? Mi a helyzet
ha blokkolódik az írási tranzakció? Mi van ha több tranzakció fut egyszerre, akkor a WRITE
állapot globális?

- a tranzakciós réteg csupán demonstrálni kívánja, hogy lehetséges tranzakcionális írási réteget helyezni a bemutatott csak-olvasható réteg fölé
- írási műveletek esetén jellemzően egyfelhasználós használatra számítunk
- főként olvasási műveletekre számítunk
- egyébként sem az írási műveletek képeznek szűk keresztmetszetet::: demo: ott vannak az adatok; tesztek: egy kezdeti művelet jellemzőit akarjuk ellenőrizni
- a későbbiekben ez továbbfejleszthető, akár egy teljeskörű tranzakciós keretrendszerré, de ez messze nem prioritás
- a csak-olvasható réteg megosztható több tranzakciós réteg alatt (a dolgozatban szereplő két tranzakciós technika ebben az irányba mutat, habár jelenleg ez még nincs megvalósítva)
- később az írási/tranzakciós rétegek megoszthatósága is tervben van (érkezett hozzám ilyen kérdés korábbról)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

3. Mitől gyorsabb a LargeInteger mint a scala BigInt? Az ábrán látjuk hogy az, bár a mérések
nincsenek leírva, de mi ennek az oka? Miért kellett sajátot írni? Miért nem volt jó a Scalas
megoldás?

- nem az volt a cél, hogy feltétlenül gyorsabban legyen, hanem hogy ne az legyen, hogy létezik egy szupergyors megoldás, mi meg egy lassabbat használunk
- azért nem vettem át egy idegen környezet megoldását, mert:
    - a BigInt felülete scalára lett tervezve, javában borzalmas (ld. például operátor-műveletek)
    - egy standalone, kényelmes megoldást akartam nyújtani, ami kompatibilis a BigInteger típussal
    - egy függetlenül fejleszthető megoldást akartam, radikálisan minimalizáltam a projekt függőségeit

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

4. A Nyers szófa kompakttá alakítás algoritmus új eredmény? Nem lehetett volna ezt külön
publikálni.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

5. A 2.1 ábrán megjelenik a SPARQL. Szemantikus adatokat is tud tárolni? Ott hogyan
vannak megvalósítva az összekapcsolások?

[RDF (Resource Description Framework), triple-store, predicate, semantic data ...]
- valóban készült egy kezdetleges SPARQL adapter (quetzal :: https://dl.acm.org/doi/10.1145/2463676.2463718)
- az erőforrások négyoszlopos táblák: numerikus egyedi kulcs, erőforrás-azonosító, predikátum, tulajdonság
- a feltüntetése demonstratív jelleg (szándékosan nagyon eltérő lekérdező-nyelveket tüntettem föl)
- nem arról van szó, hogy maga a HoloDB szemantikus adatokat tárolna, hanem a relációs modellben hozzáférhető táblák egy szemantikus nézet backendjeként ("storage" engine) is használhatók
- én ehhez a quetzal nevezetű SPARQL->SQL megoldással kísérleteztem
- nagyon kezdetleges
- a későbbiekben prioritás lesz, hogy más rendszerekkel össze lehessen kapcsolni

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

6. Az adatokat oszlop szinten tárolódnak a dolgozat alapján? Hogyan kapcsolódik ez a
megoldás az oszloporientált adatbáisokhoz? Például a Classandrához?
