...
\begin{frame}
    ...
    \begin{minipage}[c]{0.5\textwidth}
        \begin{overprint}
            ...
            \onslide<5-6>\centerline{\includegraphics[width=0.9\textwidth]{diagram/treerandom-nonindexed}}
            \onslide<7>\centerline{\includegraphics[width=0.9\textwidth]{diagram/treerandom-indexed}}
        \end{overprint}
    \end{minipage}
\end{frame}
...
\begin{frame}
    ...
    Kétlépcsős értékkiosztás alapelve: \par
...
\begin{frame}[t]
    ...
    \centering
    {\Large A \texttt{\colorbox{goldenrod!10}{([a-c]z(tt|uu)r|a[x-z])}} reguláris kifejezés szófája:}
...
\begin{frame}[t,noframenumbering]
    ...
    \textbf{A legtöbb rendszer már úgy van fejlesztve, hogy van hely bőven.
    Miért kritikus ez egy mockadatbázisnál?
    A laptopok is rendelkeznek már több GB memóriával.}
    
    \vspace{0.7cm}
    
    \begin{itemize}
        \setlength\itemsep{1em}
        \item a bő hely is véges
        \item párhuzamos futtatás
        \item az adat nem csak helyet foglal, de elő is kell állítani
        \item nem csak fejlesztéskor van szükség átmeneti adatbázisra
        \item új lehetőségek (pl. \texttt{{-}{-}dev{-}mode})
    \end{itemize}
\end{frame}

\begin{frame}[t,noframenumbering]
    \slidetitle{\sectionshorttitle}{2. kérdés}
    
    \textbf{A tranzakcióknál az írási státusznál feltételezi, hogy addig nem olvas senki?
    Mi a helyzet, ha blokkolódik az írási tranzakció?
    Mi van, ha több tranzakció fut egyszerre?
    Akkor a WRITE állapot globális?}
    
    \vspace{0.3cm}
    
    \begin{itemize}
        \setlength\itemsep{1em}
        \item igen, az alap módban egy írási tranzakció kizárólagosan fut
        \item lightweight tranzakciós megoldás
        \item főleg egyfelhasználós hozzáférésre számítunk
        \item főleg olvasási műveletekre számítunk
        \item az alap módban READ állapotban akárhány olvasás futhat egyszerre
        \item MVCC módban eldobódik az írási tranzakció, ha nem fésülhető be
        \item lényeg: nem terheljük a tábla engine-re a konkurencia kezelését
    \end{itemize}
\end{frame}

\begin{frame}[t,noframenumbering]
    \slidetitle{\sectionshorttitle}{3. kérdés}
    
    \textbf{Mitől gyorsabb a LargeInteger, mint a Scala BigInt?
    Az ábrán látjuk, hogy az (bár a mérések nincsenek leírva), de mi ennek az oka?
    Miért kellett sajátot írni, miért nem volt jó a scalás megoldás?}
    
    \vspace{0.7cm}
    
    \begin{itemize}
        \setlength\itemsep{1em}
        \item már dolgoztam a LargeInteger-en, mielőtt az új BigInt megjelent
        \item már az is elég, ha a teljesítmény összemérhető
        \item nem akartam külső függőségeket, főként ilyen alapvető típusnál
        \item a BigInt nem idiomatikus Javában (pl. \texttt{\colorbox{goldenrod!10}{n.\$less\$less(m)}})
        \item a LargeInteger támogatja a javás BigInteger metódusait
        \item a LargeInteger egy független utility, számos kényelmi metódussal
    \end{itemize}
\end{frame}

\begin{frame}[t,noframenumbering]
    \slidetitle{\sectionshorttitle}{4. kérdés}
    
    \textbf{A Nyers szófa kompakttá alakítás algoritmus új eredmény?
    Nem lehetett volna ezt külön publikálni?}
    
    \vspace{0.7cm}
    
    \begin{itemize}
        \setlength\itemsep{1em}
        \item nem találtam ennek megfelelő létező megoldást
        \item nagyon speciális céllal használom az algoritmust
        \item lehetséges, hogy megéri önállóan publikálni,
              de elsősorban a reguláris kifejezésekből épített
              virtuális értékkészletek kontextusában van értelme
    \end{itemize}
\end{frame}

\begin{frame}[t,noframenumbering]
    \slidetitle{\sectionshorttitle}{5. kérdés}
    
    \textbf{A 2.1 ábrán megjelenik a SPARQL.
    Szemantikus adatokat is tud tárolni?
    Ott hogyan vannak megvalósítva az összekapcsolások?}
    
    \vspace{0.7cm}
    
    \begin{itemize}
        \setlength\itemsep{1em}
        \item prioritás, hogy a HoloDB-t más rendszerekkel össze lehessen kapcsolni
        \item a relációs modell egy közös nevező más modellekhez
        \item nincs natív támogatás szemantikus adatokhoz
        \item egy kezdetleges driver készült a Quetzal nevezetű SPARQL keretrendszerhez
              (mely relációs adatbázist használ tárolóként)
        \item ilyenkor a schema a Quetzal tárolási struktúráját kell kövesse
    \end{itemize}
\end{frame}

\begin{frame}[t,noframenumbering]
    \slidetitle{\sectionshorttitle}{6. kérdés}
    
    \textbf{Az adatok oszlop szinten tárolódnak a dolgozat alapján.
    Hogyan kapcsolódik ez a megoldás az oszloporientált adatbázisokhoz,
    például a Cassandrához?}
    
    \vspace{0.7cm}
    
    \begin{itemize}
        \setlength\itemsep{1em}
        \item közvetlenül nem kapcsolódik
        \item NoSQL adatbázisok közül csak MongoDB lekérések mockolásával foglalkoztam,
              egyelőre az is kezdetleges,
              proof-of-concept jellegű
        \item a HoloDB valószínűleg hatékony lenne oszloporientált rendszerek backendjeként,
              különösen olyan adatoknál, ahol nincs külön permutálás
              (időbélyegek, monoton idősoros adatok stb.)
    \end{itemize}
\end{frame}
