...
\title{\LARGE HoloDB: Relációs demóadatok konzisztens on-the-fly generálása deklaratív konfigurációból}
\date{2024}
...
(például mockolás, füst-teszt, szoftverbemutató).
...
de azok indítási, működtetési és memóriaigénye is jelentős.

A pályamunkában egy újszerűnek szánt megközelítést mutatok be,
mely szükségtelenné teszi az adatok előzetes generálását.
...
Számos részmegoldás (például egyedi egész-aritmetika, kompakt szófák, kriptográfiai módszerek)
együttes alkalmazásával koherens és gyors működést biztosító
adatelérési technikák váltak lehetővé (például a virtuális indexek),
így a lekérdezési eredmények hatékonyan számíthatók on-the-fly.
...
is összemérhető a hagyományos adatbázisokkal.
A relációs modell a megfelelő adapterekkel könnyen kiterjeszthető más adatmodellekre is.

Az új megoldás további előnyök mellett
ötvözi a korábbi megoldások erősségeit, míg kikerüli a főbb hátrányokat,
különösen a generálási költséget.
A HoloDB potenciálisan széleskörűen alkalmazható adatbázismockoló eszközként,
és a meglévő folyamatokba is könnyen integrálható.
Továbbá olyan új lehetőségek is megnyílnak, mint a „dev mode”
...
\begin{tabular}{@{}p{3.8cm}@{} p{6.5cm}@{}}
    \textbf{\textit{\small ACM: Classification:}} & 1. \textit{\small Information~systems~/~DBMS~engine~architectures} \\
    ~ & 2. \textit{\small Software~and~its~engineering~/~Development~frameworks~and~environments} \\
    ~ & 3. \textit{\small Software~and~its~engineering~/~Software~prototyping}
\end{tabular}
\end{abstract}
...
Számos nagy teljesítményű szolgáltatás érhető el a piacon szintetikus adatok generálásához.
...
\vspace{-5pt}
\begin{minted}[fontsize=\small,framesep=4pt,frame=single,framerule=1.5pt,rulecolor=\color{gray!30}]{java}
...
A rekordot a \texttt{Row} típus reprezentálja,
...
A következő osztálydiagramon vázlatosan összefoglaltam az interfészeket:
...
pusztán a megfelelő \texttt{docker} parancs kiadására van szükség az indításhoz.
...
\vspace{-5pt}
\begin{minted}[fontsize=\small,framesep=4pt,frame=single,framerule=1.5pt,baselinestretch=1.25,rulecolor=\color{gray!30}]{yaml}
- name: some_column
...
(nagyrészt a scalás Spire matematikai keretrendszer \texttt{SafeLong} osztályát alapul véve%
...
\begin{itemize}
    \item A scalás osztályok javás használata nem idiomatikus.
          Például beszédes metódusnevek helyett az operátortúlterhelés kódolt nevét kell használnunk
...
\begin{tabular}{|l||r|r|r|r|r|r|r|r||} 
 \hline
 \multicolumn{1}{|>{\centering\arraybackslash}m{1.2cm}|}{\multirow{3}{*}{~}}
    & \multicolumn{4}{>{\centering\arraybackslash}c|}{\textbftt{SMALL}}
    & \multicolumn{1}{>{\centering\arraybackslash}c|}{\textbftt{LARGE}}
    & \multicolumn{3}{>{\centering\arraybackslash}c||}{\textbftt{MIXED}}
    \\
 \cline{2-9}
 
 \multicolumn{1}{|>{\centering\arraybackslash}c|}{~}
    & \multicolumn{1}{>{\centering\arraybackslash}m{1.2cm}|}{\small{\textbftt{SADD}}}
    & \multicolumn{1}{>{\centering\arraybackslash}m{1.2cm}|}{\small{\textbftt{SSIMP}}}
    & \multicolumn{1}{>{\centering\arraybackslash}m{1.2cm}|}{\small{\textbftt{SNORM}}}
    & \multicolumn{1}{>{\centering\arraybackslash}m{1.2cm}|}{\small{\textbftt{SPOW}}}
    & \multicolumn{1}{>{\centering\arraybackslash}m{1.2cm}|}{\small{\textbftt{LADD}}}
    & \multicolumn{1}{>{\centering\arraybackslash}m{1.2cm}|}{\small{\textbftt{MBTL}}}
    & \multicolumn{1}{>{\centering\arraybackslash}m{1.2cm}|}{\small{\textbftt{MADD}}}
    & \multicolumn{1}{>{\centering\arraybackslash}m{1.2cm}||}{\small{\textbftt{MCALC}}}
    \\
 
 \multicolumn{1}{|>{\centering\arraybackslash}c|}{~}
    & \multicolumn{1}{>{\centering\arraybackslash}c|}{(ns)}
    & \multicolumn{1}{>{\centering\arraybackslash}c|}{(ns)}
    & \multicolumn{1}{>{\centering\arraybackslash}c|}{(ns)}
    & \multicolumn{1}{>{\centering\arraybackslash}c|}{(ns)}
    & \multicolumn{1}{>{\centering\arraybackslash}c|}{(ns)}
 ...
 \textbftt{long}                 &         1,1  & 2,6          &          5,3  &                             16,7  & \nodata        & \nodata        & \nodata        & \nodata        \\
 \hline
 \textbftt{BigInteger}           &         36,8 &        20,9  &         79,5  &                             92,3  &          39,2  &          11,6  &         136,6  &         109,8  \\
 \hline\hline
 
 \textbftt{LargeInteger}         & \textbf{5,2} & \textbf{5,4} &  \textbf{8,1} &  \textbf{8,1}\rlap{\footnotemark} &          53,8  &          17,3  &         222,7  &         148,2  \\
 \hline
 \textbftt{BigInt}               &         6,0  &         6,9  &         22,3  &                             35,6  &  \textbf{46,0} &  \textbf{13,7} & \textbf{192,8} & \textbf{139,1} \\
 \hline
 \textbftt{SafeLong}             &         12,0 &         7,3  &         15,0  &                             24,6  &          52,3  &          16,6  &        1002,1  &        1613,2  \\
 \hline
 \textbftt{Apint}                &        558,6 &       973,5  &       2405,7  &                           2736,6  &         471,4  &          85,6  &        1768,8  &        2274,4  \\
 \hline


\end{tabular}
\end{center}
\caption[Óriás egész szám implementációk jellemzői]{
Óriás egész szám implementációk jellemzői.
\textbf{\textit{A sorok jelentése}}:
\textbftt{long}: a Java primitív \texttt{long} típusa;
\textbftt{BigInteger}: a Java beépített \texttt{BigInteger} típusa;
\textbftt{LargeInteger}: az általam implementált \texttt{LargeInteger};
\textbftt{BigInt}: a Scala beépített \texttt{BigInt} típusa;
\textbftt{SafeLong}: a scalás Spire matematikai könyvtár \texttt{SafeLong} típusa;
\textbftt{Apint}: a javás Apfloat matematikai könyvtár \texttt{Apint} típusa.
\textbf{\textit{Az oszlopok jelentése}}:
\textbftt{SMALL}: műveletek a \texttt{long} értéktartományán belül (overflow nélkül);
\textbftt{LARGE}: műveletek a \texttt{long} értéktartományán kívül;
\textbftt{MIXED}: műveletek vegyes értéktartományon (overflow is előfordul).
A második sor jelentése:
\textbftt{SADD}: $\sum_{i=1}^{7} a_i$~;~
\textbftt{SSIMP}: $\frac{(a+b)c}{d}$~;~
\textbftt{SNORM}: összetett számítás ~;~
\textbftt{SPOW}: összetett számítás, hatványozás is ~;~
\textbftt{LADD}:  $\sum_{i=1}^{5} X_i$~;~
\textbftt{MBTL}: $a+X$~;~
\textbftt{MADD}: összetett vegyes összeadás overflow-val ~;~
\textbftt{MCALC}: összetett vegyes számítás overflow-val.
A képletekben a kisbetűk \texttt{long} értéktartományán belüli,
a nagybetűk pedig azon kívüli megfelelő véletlenszámokat jelentenek.
}
\label{table:largeinteger}
\end{table}
\footnotetext{
Ha a pozitív kitevő nem túl nagy, akkor a \texttt{LargeInteger} optimális szorzást végez.
A \texttt{long} tesztjében a beépített \texttt{Math.pow} függvényt használtam,
amelyben nincs ilyen optimalizáció.
}
\vspace{-20pt}
...
Általában nem szükséges, hogy minden körhöz más hashelést használjunk,
...
A \texttt{FAV} esetében a létrehozási idő azért magas,
...
\begin{tabular}{|l||r|r|r|r|r|r|r||} 
 ...
 \multicolumn{1}{|>{\centering\arraybackslash}m{2cm}|}{~}
    & \multicolumn{1}{>{\centering\arraybackslash}m{1.6cm}|}{\begin{tabular}{c} \footnotesize{\textbftt{mem}} \\ (bytes) \end{tabular}}
    & \multicolumn{1}{>{\centering\arraybackslash}m{1.6cm}|}{\begin{tabular}{c} \footnotesize{\textbftt{mem-ex}} \\ (bytes) \end{tabular}}
    & \multicolumn{1}{>{\centering\arraybackslash}m{1.6cm}|}{\begin{tabular}{c} \footnotesize{\textbftt{paq8/4}} \\ (\%) \end{tabular}}
    & \multicolumn{1}{>{\centering\arraybackslash}m{1.6cm}|}{\begin{tabular}{c} \footnotesize{\textbftt{bzip2}} \\ (\%) \end{tabular}}
    & \multicolumn{1}{>{\centering\arraybackslash}m{1.6cm}|}{\begin{tabular}{c} \small{\textbftt{create}} \\ (ns) \end{tabular}}
    & \multicolumn{1}{>{\centering\arraybackslash}m{1.6cm}|}{\begin{tabular}{c} \small{\textbftt{at×30}} \\ (ns) \end{tabular}}
    & \multicolumn{1}{>{\centering\arraybackslash}m{1.6cm}||}{\begin{tabular}{c} \small{\textbftt{iof×30}} \\ (ns) \end{tabular}}\\
 \hline\hline
 \textbftt{FAV}         &     648 &     648 & 31,54 & 99,77 &     968 &  2~040 &   2~007 \\
 \hline
 \textbftt{MP1}         &     152 &     152 &  0,19 & 22,13 &     267 &    161 &     188 \\
 \hline
 \textbftt{FEI-F-4}     &     608 &     608 & 19,56 & 98,19 &     660 &  8~800 &   9~185 \\
 \hline
 \textbftt{FEI-S-2}     & 212~312 &     336 & 97,08 & 99,95 &     434 & 32~221 &  32~536 \\
 \hline
 \textbftt{FEI-F-1}     &     240 &     240 &  2,66 & 90,58 &     347 &  3~234 &   3~137 \\
 \hline
 \textbftt{ID}          &      40 &      40 &  0,04 & 19,49 &       2 &      6 &       6 \\
 \hline
 \textbftt{FPE1}        & 211~352 &     160 & 97,07 & 99,94 &     540 & 67~155 &  68~192 \\
 \hline
 \textbftt{BXP}         &      72 &      72 &  0,06 & 39,93 &     136 &     82 &      82 \\
 \hline
 \textbftt{BSP}         &     208 &     208 &  0,09 & 34,55 &     108 &    904 &     904 \\
 \hline
 \textbftt{MEM}         & 262~208 & 262~208 & 97,09 & 99,93 & 686~358 &     82 & 148~220 \\
 \hline
\end{tabular}
\end{center}
\caption[Permutáció-implementációk jellemzői]{
Permutáció-implementációk jellemzői.
\textbf{\textit{A sorok jelentése}}\footnotemark:
\textbftt{FAV}: az általam favorizált heurisztika (részletek a főszövegben);
\textbftt{MP1}: lineáris kongruencia;
\textbftt{FEI-F-4}: Feistel-hálózat (4 körpár, \texttt{FastHasher});
\textbftt{FEI-S-2}: Feistel-hálózat (2 körpár, SHA-256);
\textbftt{FEI-F-1}: Feistel-hálózat (1 körpár, \texttt{FastHasher});
\textbftt{ID}: identikus permutáció;
\textbftt{FPE1}: FE1 sémájú FPE;
\textbftt{BXP}: bit XOR (\texttt{BitSet}-tel);
\textbftt{BSP}: bit-permutáció (int-tömbbel);
\textbftt{MEM}: random kevert in-memory int-tömb használata.
\textbf{\textit{Az oszlopok jelentése}}
\textbftt{mem}: teljes rekurzív memóriaigény példányonként;
\textbftt{mem-ex}: u. a., de a megosztható \texttt{Mac} példány nélkül;
\textbftt{paq8/4}: méretváltozás 4-es erősségű \texttt{paq8} tömörítéssel;
\textbftt{bzip}: méretváltozás \texttt{bzip} tömörítéssel;
\textbftt{create}: példány elkészítési ideje;
\textbftt{at×30}: első 30 index értéklekérésének ideje összesen;
\textbftt{iof×30}: első 30 érték indexkeresésének ideje összesen.
}
\label{table:permutations}
\end{table}
\footnotetext{
A sorokat ár-érték arány szerint csökkenően igyekeztem rendezni,
figyelembe véve az értékeket és a vizuális eredményeket.
Az összbenyomás mérésére nem sikerült egységes képletet találnom,
így mondhatjuk, hogy a sorrend részben szubjektív.
}
...
\begin{figure}[H]
\centering
\includesvg[height=0.4\textwidth]{diagram/trie-multilevel}
...
(az egyéb paramétereket lásd az \ref{jmhBenchmarkEnvironment} alfejezetben).
...
\begin{tabular}{|l||l|l||} 
 \hline
 \multicolumn{1}{|>{\centering\arraybackslash}m{3cm}|}{~}
    & \multicolumn{1}{>{\centering\arraybackslash}m{4.5cm}|}{\textbf{Standalone}}
    & \multicolumn{1}{>{\centering\arraybackslash}m{7cm}||}{\textbf{Docker}}\\
 \hline\hline
...
\section{Általános futtatási tapasztalatok}

Felhasználói oldalról a HoloDB használata főként három dolgot jelent:
a konfigurációt, a futtatást, illetve a tesztadatbázishoz való csatlakozást.
A konfigurációról már volt szó,
a következőkben a futtatásról és csatlakozásról lesz szó.

A HoloDB szerver használható Docker konténerként vagy önálló alkalmazásként.
...
\begin{tabular}{|l||r|r||} 
 \hline
 \multicolumn{1}{|>{\centering\arraybackslash}m{1.7cm}|}{~}
    & \multicolumn{1}{>{\centering\arraybackslash}m{2.5cm}|}{\begin{tabular}{c} \makebox[0pt]{\footnotesize{\textbf{Docker image}}} \\ (MiB) \end{tabular}}
    & \multicolumn{1}{>{\centering\arraybackslash}m{2.5cm}||}{\begin{tabular}{c} \makebox[0pt]{\footnotesize{\textbf{Shadow JAR}}} \\ (MiB) \end{tabular}}\\
 \hline\hline
...
\end{tabular}
\end{center}
\caption[JAR fájl és Docker-kép mérete]{
JAR fájl és Docker-kép mérete összehasonlításban.
A \textbf{HoloDB-opt} sor egy \textit{kísérleti} disztribúciót mutat,
ahol a JAR fájlban a Proguard eszközzel kirostáltam a függőségek nem használt részeit,
illetve egy minimális \texttt{alpine}-alapú Docker-alapképet használtam minimális Java-telepítéssel.
}
\end{table}

A dockeres indítás akár egyetlen paranccsal is történhet, például:

\vspace{-5pt}
\begin{minted}[fontsize=\small,framesep=4pt,frame=single,framerule=1.5pt,rulecolor=\color{gray!30}]{shell}
docker run --rm -v sample-config.yaml:/app/config.yaml -p 3430:3430 -it miniconnect/holodb:latest
\end{minted}
\vspace{-35pt}

A \texttt{sample-config.yaml} helyén megadhatjuk a saját konfigurációs fájlunkat,
az kettőspont bal oldalán lévő \texttt{3430} helyett pedig egy egyedi portot,
amelyen az adatbázist el kívánjuk érni.

Önálló alkalmazásként forráskódból a megfelelő \texttt{gradle run} taszkkal
vagy a standalone JAR fájl indításával tudjuk futtatni.

A szerver indításához körülbelül minimum 8MB heap-méretre van szükségünk,
de a használat körülbelül 10MB-tól mondható biztonságosnak.
Ha nem töltünk be óriás értékkészleteket és egyedi működéseket,
akkor egyetlen munkamenettel csatlakozva ez a 10MB már elégséges
bonyolult lekérdezések futtatásához is akár óriás adatbázison
(például a \texttt{-Xms5M -Xmx10M -XX:+UseG1GC} JVM kapcsolókkal).

Javás programkörnyezetből a MiniConnect API-n keresztül vagy pedig JDBC-vel
tudunk csatlakozni az adatbázishoz.
Java projekt esetén beágyazott adatbázis is használható,
ami akár JPA entitásokra helyezett annotációkkal is indítható.
(A részleteket lásd a \texttt{holodb} project \texttt{README.md} fájljában,
illetve a példaprojektekben.)

Interaktívan csatlakozni a \texttt{micl} klienssel lehet
(telepítéshez lásd a \texttt{miniconnect-client} project \texttt{README.md} fájlját).
Ez a REPL kliens azonnal csatlakozik a MiniConnect API-val kompatibilis adatbázishoz
a megadott hoszton és porton (alapértelmezetten \texttt{localhost:3430}).
Gépeléskor szintaxiskiemelést, automatikus kiegészítést és parancstörténetet kapunk,
az eredménytáblák pedig szépen formázottan jelennek meg
(ugyanakkor egy fapados ASCII üzemmód is elérhető).

\begin{figure}[H]
\centering
\includegraphics[height=0.4\textwidth]{image/miniconnect-repl}
\caption{A MiniConnect REPL használat közben HoloDB szerverrel}
\end{figure}
...
Az első tesztben (\textbftt{Q-FLD}) egyetlen mező lekérését nézem.
Egy adott táblában keresem az \texttt{id} oszlop egy adott értékének pozícióját,
majd egy másik oszlop értékét a talált pozíción.
A konkrét SQL:

\begin{minted}[fontsize=\small,framesep=4pt,frame=single,framerule=1.5pt,rulecolor=\color{gray!30}]{sql}
SELECT firstname FROM students WHERE id = 30
\end{minted}

A második teszt (\textbftt{Q-REC}) hasonló,
de az összes oszlopból keresem ki az értékeket,
azaz egy teljes rekordot kérek le:

\begin{minted}[fontsize=\small,framesep=4pt,frame=single,framerule=1.5pt,rulecolor=\color{gray!30}]{sql}
SELECT * FROM students WHERE id = 30
\end{minted}

A harmadik tesztben (\textbftt{Q-CMPX}) a kulcsmezőtől eltérő mezőben keresek,
ami így több találatot eredményez.
A találatok számát 20-ra limitálom:

\begin{minted}[fontsize=\small,framesep=4pt,frame=single,framerule=1.5pt,rulecolor=\color{gray!30}]{sql}
SELECT s.lastname FROM students s WHERE s.firstname='Ian' LIMIT 20
\end{minted}

A negyedik tesztben (\textbftt{Q-TFUL}) lekérem a teljes táblát:

\begin{minted}[fontsize=\small,framesep=4pt,frame=single,framerule=1.5pt,rulecolor=\color{gray!30}]{sql}
SELECT * FROM students
\end{minted}

Az ötödik tesztben (\textbftt{Q-MULT}) több táblából gyűjtöm az adatokat,
melyeket a kulcsmezőkkel fűztem össze (\texttt{LEFT JOIN}).
Egyúttal dupla keresési feltételt is megadok két különböző táblára:
...
\begin{tabular}{|l||r|r|r|r|r||} 
 \hline
 \multicolumn{1}{|>{\centering\arraybackslash}m{1.7cm}|}{~}
    & \multicolumn{1}{>{\centering\arraybackslash}m{1.3cm}|}{\begin{tabular}{c} \makebox[0pt]{\footnotesize{\textbftt{Q-FLD}}} \\ ({\textmu}s) \end{tabular}}
    & \multicolumn{1}{>{\centering\arraybackslash}m{1.3cm}|}{\begin{tabular}{c} \makebox[0pt]{\footnotesize{\textbftt{Q-REC}}} \\ ({\textmu}s) \end{tabular}}
    & \multicolumn{1}{>{\centering\arraybackslash}m{1.3cm}|}{\begin{tabular}{c} \makebox[0pt]{\footnotesize{\textbftt{Q-CMPX}}} \\ ({\textmu}s) \end{tabular}}
    & \multicolumn{1}{>{\centering\arraybackslash}m{1.3cm}|}{\begin{tabular}{c} \makebox[0pt]{\footnotesize{\textbftt{Q-TFUL}}} \\ ({\textmu}s) \end{tabular}}
    & \multicolumn{1}{>{\centering\arraybackslash}m{1.3cm}||}{\begin{tabular}{c} \makebox[0pt]{\footnotesize{\textbftt{Q-MULT}}} \\ ({\textmu}s) \end{tabular}}\\
 \hline\hline
 
 \textbf{HoloDB/}\textbftt{EXT} & 43,00 & 180,35 & 1~981,98 & 1~514~177 & 6~818 \\
 \textbf{HoloDB/}\textbftt{DEF} & 24,66 & 130,08 &   225,17 & 1~143~721 & 3~592 \\
 \textbf{HoloDB/}\textbftt{MIN} & 22,82 &  20,22 &    44,58 &    25~909 &   365 \\
 \textbf{H2}                    & 12,77 &   9,73 &    17,60 &     1~965 &   177 \\
 \textbf{MariaDB}               & 47,87 &  46,27 &    73,02 &     3~393 & 1~429 \\
 \hline

\end{tabular}
\end{center}
\caption[Lekérdezési tesztek eredménye]{
Lekérdezési tesztek eredménye különböző adatbázisokkal.

\textbf{\textit{A sorok jelentése}}
(pontos verziók fentebb a főszövegben):
\textbf{HoloDB/}\textbftt{EXT}: beágyazott HoloDB legjobb minőségű paraméterekkel;
\textbf{HoloDB/}\textbftt{DEF}: beágyazott HoloDB alapértelmezett paraméterekkel;
\textbf{HoloDB/}\textbftt{MIN}: beágyazott HoloDB minimális paraméterekkel;
\textbf{H2}: beágyazott H2;
\textbf{MariaDB}: MySQL/MariaDB szerver.
\textbf{\textit{Az oszlopok jelentése}}: Lásd a főszöveget.
}
\label{table:queryComparison}
\end{table}
...
\begin{table}[H]
\captionsetup{justification=justified, singlelinecheck=false, margin=2.5cm}
\begin{center}
\begin{tabular}{|l||r|r|r||} 
 \hline
 \multicolumn{1}{|>{\centering\arraybackslash}m{3cm}|}{~}
    & \multicolumn{1}{>{\centering\arraybackslash}m{2cm}|}{\begin{tabular}{c} \makebox[0pt]{\footnotesize{\textbftt{RO-SIMP}}} \\ (s) \end{tabular}}
    & \multicolumn{1}{>{\centering\arraybackslash}m{2cm}|}{\begin{tabular}{c} \makebox[0pt]{\footnotesize{\textbftt{RO-CMPX}}} \\ (s) \end{tabular}}
    & \multicolumn{1}{>{\centering\arraybackslash}m{2cm}||}{\begin{tabular}{c} \makebox[0pt]{\footnotesize{\textbftt{WR-CMPX}}} \\ (s) \end{tabular}}\\
 \hline\hline
 
 \textbf{HoloDB/}\textbftt{EXT} & 6,79 & 7,10 & 12,16 \\
 \textbf{HoloDB/}\textbftt{DEF} & 6,36 & 6,56 & 11,09 \\
 \textbf{HoloDB/}\textbftt{MIN} & 4,60 & 4,76 &  7,54 \\
 \textbf{H2}                    & 3,92 & 4,02 &  6,07 \\
 \textbf{MariaDB}               & 5,10 & 5,43 &  8,38 \\
 \hline

\end{tabular}
\end{center}
\caption[Integrált tesztek eredménye]{
Integrált tesztek eredménye különböző adatbázisokkal.

\textbf{\textit{A sorok jelentése}}: Lásd a \ref{table:queryComparison} táblázatot.

\textbf{\textit{Az oszlopok jelentése}}: Lásd a főszöveget.
}
\label{table:integratedComparison}
\end{table}


\section{Adatgenerálás, indítás és tárigény}

A HoloDB esetében nincs szükség adatgenerálásra,
ám konfigurációjának méretét,
indítási idejét\footnote{
  Jelenleg indításkor a HoloDB konfigurációjában megadott összes oszlop
  szükséges adatstruktúrái legenerálódnak.
  Ez természetesen erősen optimalizálható.
  Ha ezek a generálások csak később, on-demand jelleggel futnának,
  az indítási időt szinte a JVM indulási idejére lehetne redukálni.
  Fejlesztői gépeken még ezt is ki lehetne küszöbölni egy daemon futtatásával.
}
és tárhelyigényét összemérhetjük más adatbázisokéval.
Természetesen ezek azok az aspektusok, ahol a HoloDB diadalmenetét várjuk,
ez adja az egész projekt értelmét.
...
A HoloDB alapértelmezett beállításait használtam.
...
\begin{tabular}{|l|l||r|r||r|r||} 
 ...
 \multicolumn{1}{|>{\centering\arraybackslash}m{2.5cm}|}{~}
    & \multicolumn{1}{>{\centering\arraybackslash}m{3cm}||}{\textbf{Generátor}}
    & \multicolumn{1}{>{\centering\arraybackslash}m{2cm}|}{\begin{tabular}{c} \makebox[0pt]{\footnotesize{\textbf{Generálás}}} \\ (s) \end{tabular}}
    & \multicolumn{1}{>{\centering\arraybackslash}m{2cm}||}{\begin{tabular}{c} \makebox[0pt]{\footnotesize{\textbf{Indítás}}} \\ (s) \end{tabular}}
    & \multicolumn{1}{>{\centering\arraybackslash}m{2cm}|}{\begin{tabular}{c} \makebox[0pt]{\footnotesize{\textbf{Konfiguráció}}} \\ (B) \end{tabular}}
    & \multicolumn{1}{>{\centering\arraybackslash}m{2cm}||}{\begin{tabular}{c} \makebox[0pt]{\footnotesize{\textbf{Tárigény}}} \\ (MiB) \end{tabular}}\\
 \hline\hline
...
Kisméretű adathalmaz (7~200 rekord) generálása különböző megoldásokkal.
}
\end{table}
...
Nagyméretű adathalmaz (11~002~000 rekord) generálása különböző megoldásokkal.
}
\end{table}
...
Több szálon futó klienssel teszteltem,
hogyan reagál a HoloDB szerver tömegesen beérkező változó lekérdezésekre.
A konfiguráció ugyanaz volt,
mint az előbbi nagyméretű generálás esetében.

A szervert önálló alkalmazásként futtattam a \texttt{-Xms8M -Xmx50M -XX:+UseG1GC} JVM argumentumokkal.
A kliens egy egyszerű Java program volt,
mely 45 szálon 10 másodpercig küldött kéréseket.
Minden szál 0,2 másodperc szünetet tartott két kérés között.
A szálak háromféle SQL-t küldtek (mindegyiket 15 szál):

\begin{minted}[fontsize=\small,framesep=4pt,frame=single,framerule=1.5pt,rulecolor=\color{gray!30}]{sql}
SELECT * FROM employees WHERE id = ?
SELECT e.*, c.identifier FROM coupons c LEFT JOIN employees e ON c.employee_id=e.id WHERE c.id = ?
SELECT * FROM employees ORDER BY lastname ASC, firstname DESC LIMIT 20
\end{minted}

A kérdőjelek helyére kérésenként véletlenszámok kerültek behelyettesítésre.

A szerver a kéréseket jól fogadta, bár a hálózati átvitel egyelőre meglehetősen lassú.
Egy-egy kérés end-to-end feldolgozási ideje az első két SQL esetében kb. 0,05 másodperc volt,
a harmadik kérés esetében akár 4-5 másodpercre is megnyúlt.
A párhuzamos futtatáson és a hálózati átvitelen tehát még bőven van fejleszteni való.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{image/load-test}
\caption[Terheléses teszt eredménye]{
Terheléses teszt eredménye.
Az árnyékolt terület mutatja a kliens futási ablakát.

(A grafikon a VisualVM eszközből mentett képek alapján készült.)
}
\end{figure}


\chapter{Speciális használati példák}
...
A tábla mérete és egyéb jellemzői a táblanév alapján kerülnek meghatározásra,
...
\begin{enumerate}
    ...
    \item \textbf{Opcionálisan írható.} Valóban;
          ...
          melyeket a \texttt{DiffTable} dekorátorral valósít meg,
...
Kiemelendő még a hálózati átvitel átviteli sebességének javítása,
ami minden alkalmazást értint, amely nem beágyazottan futtatja a tesztadatbázist.
...
\begin{algorithmic}[1]
    ...
    \ForAll{\texttt{nextInput} \textbf{in} \texttt{nextInputs}}
        \State \texttt{normalizedNextInputs $\gets$ \Call{mergeIntoNormalizedMap{\textunderscore}ExternalHelper}{normalizedNextInputs,nextInput}}
    \EndFor
