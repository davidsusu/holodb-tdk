...
\textbftt{SNORM}: $\left| \frac{e^2}{2}x \right|$~;~
\par\textbftt{SPOW}: $((a+b)(a-b) + 1)^3 \mod ((c \lan d) \lor e) ~+~ -\max (b, d+ \min(a,\frac{e}{a}))$~;~
\par\textbftt{LADD}: $((a+b)(a-b) + 1) \mod ((c \lan d) \lor e) ~+~ -\max (b, d+ \min(a,\frac{e}{a}))$~;~
\textbftt{MBTL}: $xxx$~;~
\textbftt{MADD}: $xxx$~;~
\textbftt{MCALC}: $xxx$~.
