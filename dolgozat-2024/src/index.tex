...
A frontend pedig úgy van kialakítva, hogy ennek hatására gazdaságosan újratöltse az éppen megjelenített nézetet.
...
Az automatikus újratöltés egy radikalizált változatát \textit{zero mode}-nak nevezem,
melynek használatakor konfigurációs fájlra sincsen szükség.
Ekkor ugyanis mindent a beérkező kérések alapján fogunk generálni.

Egy-egy lekérdezés implicite megmondja, milyen adatszerkezetre számít.
Például a `SELECT firstname FROM names WHERE birtyear=1987 LIMIT 10` lekérdezés.





\todoref{Zero mode: igazítani, garancia-szintek}
