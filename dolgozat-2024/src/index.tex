...
Így a beépített \texttt{BigInteger} típust hamar elvetettem,
...
a \texttt{LargeInteger} \textcolor{red}{valamivel} gyorsabb.
...
így \textcolor{red}{enyhén} több memóriát is használ.
...
\todoref{LargeInteger: miért nem a BigInt? mérési módsz., JMH, több mérés, mem., optim., valamivel}
...
\subsubsection{Permutációk átméretezése}

A permutáció-implementációk egyes típusai nem támogatnak tetszőleges méretet
(például csak 2-hatványokat).
Ekkor egy megfelelő dekorátor objektum alkalmazása szükséges,
...
Általános módszerek is felmerülnek az átméretezésre,
melyekkel elkerülhetők a rejection sampling problémái.
A legegyszerűbb, ha nem a következő legkisebb,
hanem az előző legnagyobb támogatott méretet vesszük,
és csak az ez alatti bemeneteket permutáljuk.
Ez a módszer sokféleképpen továbbfejleszthető.
Például úgy, hogy két vagy több permutációt futtatunk le eltoltan a teljes intervallum támogatott méretű szakaszaira,
melyek egészében lefedik a teljes intervallumot.
Egy gyors utólagos permutálással gondoskodhatunk róla, hogy az intervallumok között is keveredés legyen.
Ha a szakaszok között itt-ott van is átfedés,
akkor is csak ritka esetben kell kétszer (és soha nem többször)
lefuttatni az esetleg költséges permutációt.
...
A 65~536 érték 16 bites, leírtehát 2 bájtos reprezentációkkal,
A kimenet kiírható egy-egy 131~072 bájt méretű fájlba,
ezt a fájlt tömörítettem.
Két erős tömörítést használtam, a \texttt{bzip} programot,
illetve a \texttt{paq8} programot 4-es erősséggel.
...
Ez valószínűleg elkerülhető, egyelőre ezzel nem foglaloztam.
...
\multicolumn{1}{|>{\centering\arraybackslash}m{2cm}|}{~}
    ...
    & \multicolumn{1}{>{\centering\arraybackslash}m{1.6cm}|}{\begin{tabular}{c} \small{\textbftt{create}} \\ (bytes) \end{tabular}}
