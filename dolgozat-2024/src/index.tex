...
\todoref{Chapter: Empirikus tapasztalatok, összehasonlítás}
...
\texttt{courses} (4 oszlop, 30 rekord) és
\texttt{students} (4 oszlop, 10~000 rekord).
\texttt{course\_student} (kapcsolótábla, 30~100 rekord).
(a konkrét konfigurációs fájlt lásd a tesztrepóban).
...
Az első tesztben (\textbftt{Q-FLD}) egyetlen mező lekérését néztem.
Ekkor egy adott táblában keressük az \texttt{id} oszlop egy adott értékének pozícióját,
és visszaadjuk egy másik oszlop értékét ezen a pozíción.
Konkrétan ezt az SQL-t futtattam:
...
de az összes oszlopból kerestem ki az értékeket,
azaz egy teljes rekordot kértem le:
...
A harmadik tesztben (\textbftt{Q-CMPX}) a kulcsmezőtől eltérő mezőre kerestem rá,
ami így több találatot eredményez.
A találatok számát 20-ra limitáltam:
...
A negyedik tesztben (\textbftt{Q-TFUL}) lekértem a teljes táblát:
...
Az ötödik teszt (\textbftt{Q-MULT}) több táblából gyűjt adatokat,
melyeket a kulcsmezőkkel fűztem össze (\texttt{LEFT JOIN}).
Egyúttal dupla keresési feltételt is megadtam:
...
A szerveres kontextust nézve, a HoloDB Docker-képének mérete XXXXXXXXX, mint a MariaDB-é;
XXXX a base image változtatásával,
...
a MariaDB-t pedig a `useServerPrepStmts=false&cachePrepStmts=false` opciókkal
