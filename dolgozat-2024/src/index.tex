...
A szoftver Java környezetben készült, ezt egyes esetekben érdemes lehet figyelembe venni.
...
\begin{algorithm}[H]
\caption[Nyers szófa kompakttá alakítása]{\textbf{\texttt{convertRawTrieToCompact(parentNodeValue,anchors,children)}} -- \par -- Nyers szófa kompakttá alakítása}\label{algorithm:regex_wordtree_create_compact}
\begin{algorithmic}[1]
    \scriptsize
    \State \Comment{\texttt{parentNodeValue}: regex AST érték, \texttt{anchors}: horgonyok eddigi szekvenciája, \texttt{children}: következő gyermekelemek}
    \State \texttt{nextInputs : Array<Trie>}
    \State \texttt{normalizedNextInputs : SortedMap<CharClassAstNode|EowAstNode, Array<Trie{>}{>}}
    \State \texttt{nextInputs $\gets$ \Call{collectNextInputs{\textunderscore}Helper}{parentNodeValue,anchors,children}}
...
\begin{algorithmic}[1]
    ...
    \ForAll{\texttt{childNode} \textbf{in} \texttt{children}}
        \If{\texttt{childNode.value} \textbf{is} \texttt{AnchorAstNode}}
            ...
            \State \texttt{subChildren $\gets$ collectNextInputs(parentNodeValue, subAnchors, childNode.children)}
            \State \texttt{resultQueue.enqueueAll(subChildren)}
        \ElsIf{\texttt{childNode.value} \textbf{is} \texttt{EmptyAstNode}}
            \State \texttt{subChildren $\gets$ collectNextInputs(parentNodeValue, anchors, childNode.children)}
            \State \texttt{resultQueue.enqueueAll(subChildren)}
