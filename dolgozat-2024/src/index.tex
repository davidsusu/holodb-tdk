...
\par\textbftt{SNORM}: $\left| ((a+b)(a-b) + 1)^3 \mod ((c \lan d) \lor e)\right| ~+~ -\max (b, d+ \min(a,\frac{e}{a}))$~;~
\par\textbftt{SPOW}: $\left| ((a+b)(a-b) + 1) \mod ((c \lan d) \lor e)\right| ~+~ -\max (b, d+ \min(a,\frac{e}{a}))$~;~
\par\textbftt{LADD}:  $\sum_{i=1}^{5} A_i$~;~
\textbftt{MBTL}: $xxx$~;~
\textbftt{MADD}: $xxx$~;~
