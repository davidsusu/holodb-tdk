...
Így például az integrációs tesztek jelentős erőforrást fognak igénybe venni
...
amelynek ráadásul csak egy töredéke lesz érintve egy-egy teszt során.

Számos esetben tehát szükségünk van egy működő adatbázisra,
de valójában nincs szükségünk igazi adatokra.
Ez egy paradox helyzet, és felveti annak kérdését, hogy ezekért az átmeneti adatokkal
azonos súlyban van-e az erőforrásigény.
Egyáltalán, van valami abszurd abban, amikor ilyen nagy mennyiségű adatot
könnyedén lematerializálunk, majd nagyrészt használatlanul eldobjuk.
Felmerül a kérdés: vajon elkerülhető mindez?
Más egyértelmű megoldás híján azonban inkább a meglévő módszerek tökéletesítésére törekszünk.
Ördögi kör.

Amikor egy szoftver kezdeti fázisában az adatbázis szerkezete is gyakran változik,
a teszteléskor az adatbázist újra létre kell hozni,
vagy igazítani a megváltozott adatszerkezethez.
Mindkettő nehézségekkel és erőforráshasználattal jár.
Sokkal egyszerűbb lenne az adatbázis szerkezetét egyetlen konfigurációs fájl,
vagy akár az alkalmazásban lévő adatobjektumok változásával függő viszonyba helyezni.

A dolgozatban elsősorban ezekre a problémára kínálok egy újszerű megoldást.
...
empirikus benyomásával tudom alátámasztani.

Számomra meglepő, hogy egy ilyen problémára,
mely ennyire napi szinten érinti fejlesztők, tesztelők, tanulók és mások életét,
egyelőre csak a legszofisztikáltabb kerülőmegoldások születtek.
Kicsivel lentebb megpróbálom majd megtippelni ennek az okát.

A következőkben végigveszek néhány megközelítést,
melyek valamennyire kapcsolódnak a fenti problémához,
hasonló akarnak megoldani.
...
melyek bizonyos gyakori entitástípusok adatait (pl. személynevek, e-mail, dátum) szolgáltatja.

Ez a megközelítés természetesen nem is törekszik arra,
hogy koherens eredményt adjon több lekérés esetén,
tehát csak olyan esetben használható, ahol a lekérések nem épülnek egymásra.
...
Erre a kérdésre egy rövidebb és egy hosszabb válasszal szolgálhatok.
Mindegyik tényállítás annyiban, hogy önmagában igaz,
és puszta tipp atekintetben, hogy mennyire okozója a vizsgált helyzetnek.

A rövid válaszom, hogy a rohanó világban, különösen a lokális optimumra törekvő fejlesztők világában,
az egyértelmű alternatív megoldás hiánya szükségszerűen
a bevett kerékvonalak továbbmélyítését eredményezi.

A hosszabb válaszban néhány tényező összejátszását sorolnám föl:
...
Ez egy már működő és kipróbálható szoftver, melynek a HoloDB nevet adtam,
még a kezdetekkor ugyanis úgy gondoltam rá,
...
Ezt meg lehetne oldani úgy is, hogy pusztán determinisztikusan generáljon
egy pszeudorandom értéket egy-egy rekordmezőre,
így kereséskor a teljes tábla szkennelése szükséges.
A lényegi trükk azonban az hogy általában nincs szükség ilyen költséges szkennelő műveletekre,
az oszlopok ugyanis alapértelmezetten virtuálisan indexeltek.
Ehhez meg kell oldani, hogy értéklekérés és keresés oda-vissza egybecsengően működjön,
ha egy keresett adatot valahol megtaláltunk, akkor ugyanott lekérve meg is kapjuk, és viszont.
Ehhez részben a kriptográfiából ismert módszereket használtam.

A végeredmény egy skálázható és testreszabható nyílt-forráskódú adatbázis-mockoló eszköz,
melyhez számos kiegészítő eszköz is készült
(csak néhány példa ezekre: REPL; networking; JDBC és JPA támogatása, Docker, haladó értékkészletek).
...
bár a számos példa alapján ez szinte néhány másolással is megoldható,
de ha ez sem lenne elég, akár generálható is.

Bár a teljes architektúra tárgyalása is sok érdekességet tudna bemutatni,
a tárgyalás során a fókuszt egyértelműen a relációs storage API bemutatása adja majd,
ez ugyanis a szűk keresztmetszet, amely a megoldás újszerűségéhez a leginkább hozzájárul,
és egyúttal a prototípusban is ezek a legkidolgozottabb komponensek.

Úgy gondolom, a forráskód a dolgozatban konkrétan bemutatott komponenseken és megoldásokon
túl is sok izgalmas dolgot rejt,
ezért jó szívvel ajánlom a szoftver Git-tárolóját,
melyet a dolgozat legvégén linkeltem is.

\section{Alkalmazhatóság}

Fentebb már tisztáztam, mi volt a fő motiváció a szoftver elkészítésében,
de további alkalmazhatóságokat is lehet találni.
A következő listában csak a legmarkánsabbakat sorolom föl:

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% TODO

\begin{itemize}
    \item \textbf{ASDF.} XXXXXXXXXXXXXXXXXXXXXXx.
    \item \textbf{FDSA.} XXXXXXXXXXXXXXXXXXXXXXx.
\end{itemize}

% azonnali indulás, minimális memóriafogyasztás

%Alkalmazhatóság: serverless, dev mode, JPA annotációk,
%    letapogatás és renderelés, AI általi generálás, zero mode
...
\section{A dolgozat programja}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% TODO

\section{Tézisek}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% TODO: a végén nyugtázni is!!!

\begin{itemize}
    \item \textbf{Tézis 1: ASDF.} XXXXXXXXXXXXXXXXXXXXXXx.
    \item \textbf{Tézis 2: FDSA.} XXXXXXXXXXXXXXXXXXXXXXx.
\end{itemize}
...
mivel jelentősen a Feistel-hálózatra történő újrahívások számát.
...
melyekből már nem érhető el további csomópont. \parencite{AppelScrabble1988}
...
\algorithmref{regex_wordtree_iterate}
...
\algorithmref{regex_wordtree_create_multilevel}
...
\algorithmref{regex_wordtree_getfrom_multilevel}
...
A dolgozatban elmondottak igazolják, hogy a felállított tételeink teljesültek:
...
\begin{algorithm}[H]
\caption{Feistel-hálózat last-zero változat}\label{algorithm:feistel}
\begin{algorithmic}[1]
    \State first line
...
\begin{algorithm}[H]
\caption{Nyers szófa összeállítása}\label{algorithm:regex_wordtree_create_raw}
\begin{algorithmic}[1]
    \State first line
    \Statex continuing first line
    \State \Call{Proc}{a1, a2}
...
\phantomsection
\addcontentsline{toc}{section}{Kompakt szófa konverziója nyers szófából}

\begin{algorithm}[H]
\caption{Kompakt szófa konverziója nyers szófából}\label{algorithm:regex_wordtree_create_compact}
\begin{algorithmic}[1]
    \State first line
    \Statex continuing first line
    \State \Call{Proc}{a1, a2}
...
\phantomsection
\addcontentsline{toc}{section}{Szavak iterációja kompakt szófán}

\begin{algorithm}[H]
\caption{Szavak iterációja kompakt szófán}\label{algorithm:regex_wordtree_iterate}
\begin{algorithmic}[1]
    \State first line
    \Statex continuing first line
    \State \Call{Proc}{a1, a2}
...
\phantomsection
\addcontentsline{toc}{section}{Többszintű fa építése}

\begin{algorithm}[H]
\caption{Többszintű fa építése}\label{algorithm:regex_wordtree_create_multilevel}
\begin{algorithmic}[1]
    \State first line
    \Statex continuing first line
    \State \Call{Proc}{a1, a2}
...
\phantomsection
\addcontentsline{toc}{section}{Lekérés a többszintű fából}

\begin{algorithm}[H]
\caption{Lekérés a többszintű fából}\label{algorithm:regex_wordtree_getfrom_multilevel}
\begin{algorithmic}[1]
    \State first line
    \Statex continuing first line
    \State \Call{Proc}{a1, a2}
