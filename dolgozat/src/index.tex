...
amely egy tetszőlegesen nagy méretű, konzisztens és kereshető adattárat imitál,
miközben valójában mindebből semmi sem létezik,
az egész mintegy illúzió.
Hogy ezt az illúziót elérjük, egyfelől meg kell találni azt az univerzális modellt,
...
illetve lehetőleg ezzel konzisztensen az értékek kereshetők is.
...
\longcomment{A megvalósítás alapjai...}{

  Ha az adatelérést objektumok...
  interfészek, erőteljes bővíthetőségű
  ...
  - Végtelenített virtuális világok
  - A fakerek limitációi
  - Néhány visszatérő probléma közös nevezője
  - A fakerek limitációi
  - Virtuális mockolás: hézag jelei új termék számára?
  - ...
  - Felépítés
  - Konfiguráció
  ...
  - determinisztikus, de a seed-del variálható//összekeverhető
  - Relational data as a serverless function
  - SQL, NoSQL, GraphQL, REST etc.
  - miniconnect, minibase, REPL
  - a konfiguráció előfeldolgozható (pl. yaml + go template, mint a helmnél)
  - valós adatbázis feltöltése innen
  - konfiguráció generálása
    - valós adatbázis egyszerű feldolgozásával
    - valós adatbázis intelligens feldolgozásával (pl. evolúciós algoritmussal)
    - valós adathalmazokon tanított AI által
}
...
\begin{figure}[H]
  ...
  \begin{minipage}[b]{0.4\textwidth}
    \includegraphics[width=\textwidth]{image/permutation-fpe1.png}
    \caption{FPE1 permutation}
  \end{minipage}
  \hspace*{\fill}
  \begin{minipage}[b]{0.4\textwidth}
    \includegraphics[width=\textwidth]{image/permutation-mp1.png}
    \caption{Modulo permutation}
  \end{minipage}
  \hspace*{\fill}
  ...
  \hspace*{\fill}
  \begin{minipage}[b]{0.4\textwidth}
    \includegraphics[width=\textwidth]{image/permutation-feif1.png}
    \caption{Feistel network 1 + fast hash}
  \end{minipage}
  \hspace*{\fill}
  \begin{minipage}[b]{0.4\textwidth}
    \includegraphics[width=\textwidth]{image/permutation-feis2.png}
    \caption{Feistel network 2 + SHA256}
  \end{minipage}
  \hspace*{\fill}
\end{figure}
...
Gyakran egy adott értékkészlet elemeiből egy $n$ hosszúságú rendezett listát akarunk képezni.
Az eloszlás (beleértve, hogy valamely érték egyáltalán előfordul-e) teljesen implementációfüggő lehet. Mivel az értékkészlet maga is egy $k$ hosszúsága lista, a további leképezési kompozíciók szempontjából rugalmasabb, ha az értékindexeket, azaz a $[0..k)$ egész-intervallum számait tesszūk a listába.
...
A rendezettség miatt a második kérdésre mindig egy összefüggő sáv a válasz. Ha az értékindex nem szerepel, akkor ez egy zéróhosszú sáv azon a helyen, ahová a rendezés szerint beszùrható lenne.

Értelemszerűen értéksávokra is tudunk keresni, hiszen az értéksáv szélső értékeinek értékindedeire keresve megkapjuk a képzett sáv széleinek sorindexeit is.

Ezeket az elvárásokat formalizálja a \texttt{Monotonic} interfész.

\todo[inline]{Monotonic implementációi}

\subsection{A \texttt{TreeRandom} és kapcsolódó API-k}

Az adatbázist egy hierarchikus képződményként is fölfoghatjuk,
ahol a sémákon belül táblák, azon belül oszlopok,
azon belül pedig sorindexekkel megjelölt mezők foglalnak helyet
...
\begin{itemize}
   ...
   \item eltérő példányoknál jellemzően teljesen független, különböző kimenetet ad, azonosaknál ugyanazt
   \item egy kulcs bájtsorozat megadásával visszaad egy újabb példányt, mely az eggyel alacsonyabb szintnek felel meg
   \item ugyanazzal a kulcs bájtsorozattal egyenlő példányt ad vissza
\end{itemize}

\todo[inline]{soft követelmények külön? csupa 1-es csak ezeket nem teljesíti}
...
amennyi a szám elfogulatlan kiválasztásához szükséges (rejection sampling).\footnote{
  ...
  Ha a visszaadott bitgenerátor például csupa 1-est generál
  (ami nem teljesít ugyan minden követelményt, de fapados implementációként teljesen valid),
  könnyen végtelen ciklus alakulhatna ki.
}

\todo[inline]{BitSource és tovbbi interfészek}

\todo[inline]{TreeRandom implementációi}

\subsection{A storage API}

\todo[inline]{A storage API}
...
hogy előre ismert véges értékkészlet elememeit szeretnénk viszontlátni az oszlopban,
...
\todo[inline]{Összetett értékek kezelése}
...
A megoldás a reguláris kifejezések lehetőségeinek csak valamilyen limitált részhalmazát fogja támogatna.
