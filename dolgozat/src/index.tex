...
(A sűrűn benépesített világok szomszédos szeleteire vonatkozó
összeillési és interpolációs problémáktól
az egyszerűség kedvéért most tekintsünk el.)

Egy közismert korai példa hasonló módszert alkalmazó szoftverre
...
\longcomment{Perlin-zaj, simplex zaj stb.}{
sajátos előnyeik
szempont: ha ennek az adatok közelisége a speciális erőssége,
  akkor egy relációs adatbázishoz készíthető hasonló megoldásnak az indexeltség
    (utóbbit később is hozzá lehet tenni, visszutalásként)
egy illusztráció
}

Felületek generálása esetén gyakran használnak valamilyen interpolált eljárást, például Perlin-zajt,
...
Mivel ez Java nyelven íródott, a továbbiakban esetenként implicite felteszem, hogy Java környezetben vagyunk.
...
mivel a relációs séma egy teljesen általános normalizációs segédeszköz.

Hamar arra a megállapításra jutottam, hogy a releváns értelemben legalapvetőbb objektum az oszlop.
Az oszlop alatt azonos típusú adatokat találunk,
az első feladat mindig ezen adatok értékkészletének megadása.
...
Ezeket úgy kell megvalósítani, hogy a végső értéklista n-edik eleme is gyorsan lekérhető legyen,
valamint egy adott érték vagy értéksáv előfordulási pozíciói is,
utóbbi esetben rendezetten.
...
hogy az adatokat visszakereshetően szétszórjuk.
...
Gyorsan számítható invertálható permutációk például modulo szorzásbóll kiindulva valósíthatók meg.
Költségesebb, de igazán véletlenszerű összevisszaságot bitzosító invertálható permutációkat
különféle kriptográfiai módszerekkel állítok elő.
Többéle módszer kipróbálása után (pl. Format Preserving Encryption) végül
...
Az adatbázis méretére vonatkozóan sem akartam megkötéseket tenni.
Nem lett volna célszerű explicit ellenőrzések tömegét beépíteni a problémás esetek földerítésére.
...
Bár létezik egy-két megoldás, mely ugyanezt a problémát célozza (például a Scala \texttt{BigInt} típusa),
végül egy egyedi típus implementálása mellett döntöttem,
mellyel sikerült minden alternatíívánál jobb performanciát elérnem.
...
ilyen például az Apache Calcite, ami lehetővé tette egy teljeskörű SQL-szerver kiépítését.
...
[TODO]
  ...
  a relációs adatbázis a közös nevező, már eleve egy normálforma (amit a relációs normálformák csak tovább szofisztikálnak)
- adja magát, hogy az oszlop legyen az alapentitás (ez fogja össze a hasonló adatokat, ami önmagában egy fókusz), ide kerül a főfókusz, az oszlopok és táblák közötti viszonyokat is ebből kiindulva tudjuk majd megvalósítani
- a rekordok egységessége? a relációs-volt... mi tesz egy relációt valóságossá vagy épp nem?
- ha egy oszlopot kereshető akarunk tenni, indexet kell hozzá szolgáltatni, tehát a lekérdezhetőség kétirányú
- különféle indexek...
- összevissza adatokat akarunk; hogyan lehet ezt megvalósítani?
- permutációval tudjuk keverni --> kriptográfiai megfordítható permutációk
- monotonic
- egyes adatoknál van egy adott értékkészlet, másoknál ez virtuális (pl. regex)
- hogy az óriás számosságokat jól kezeljük, kell a hatékony LargeInteger (performancia-teszt bemutatása?)
