...
További probléma, hogy az adatok megszerzéséhez az éles adatbázist kell felkeresni,
ami egyfelől az éles környezet nemkívánatos terhelését jelenti,
másrészt az oda való beauthentikálás konkrét megoldástól függően biztonsági kockázatot rejthet.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}
        \begin{axis}[
                symbolic x coords={Preparálás,Indítás,Tesztek,Ebből SQL},
                ybar,
                axis on top,
                width=0.9\textwidth,
                height=11cm,
            ...
            \legend{MariaDB (generálás),MariaDB (anonimizálás),HoloDB}
        \end{axis}
    \end{tikzpicture}
    \caption[Futási idők összehasonlítása]{
        A különböző megközelítések átlagos futási idejének összehasonlítása
    }
\end{figure}

A használt tesztszcenárió rendkívül egyszerű,
és elsősorban arra szolgált, hogy egy nagyságrendi képet kapjak a teljesítmények arányáról.
Egy JPA-orientált webes \textit{Micronaut} alkalmazást futtattam webszerverként,
ennek REST végpontjait hívtam meg egy külső, szint Java-alapú kliensből.
Az adatbázisban tantárgyak, a hozzájuk rendelt kurzusok, valamint hallgatók szerepeltek,
utóbbiak nagy számban ($10~000$ rekord).
Ezeken végeztem CRUD műveleteket,
illetve egy statisztikai végpontról időnként plusz információkat kértem le,
ezeket valamivel összetettebb SQL-lekérdezések állították elő.
50 független futtatás eredményét átlagoltam.

A generálásos és anonimizálásos módszerhez is \textit{MariaDB} adatbázist használtam.
A generálási folyamatot egy Python nyelven írt szkripttel vezéreltem,
az adatokat a Pythonhoz elérhető \textit{Faker} könyvtárral generáltattam.
Az anonimizálásnál egy ilyen előre legenerált adathalmazból indultam ki,
és minden szöveges értéket következetesen egy másikra cseréltem adatbázislekéréseken keresztül,
szintén python környezetből.

A preparálási időbe a generálásos módszer esetében a generáló szkript
teljes futási idejét számítottam bele.
Az anonimizálásnál a meglévő adatbázishoz való kapcsolódás indításától
mértem az időt a kapcsolat zárásáig,
maga az anonimizálás a kettő között aktív adatbáziskapcsolattal zajlott.
A HoloDB esetében preparálásra nincs szükség, így itt zéró érték szerepel.

Az indítási időt az adatbázisszerver és az alkalmazás elindulásától
a sikeres kapcsolódásig számítottam.

Hogy valószerű eredményt kapjak, a teszteket REST API-n keresztül futtattam.
Ez overheadet jelentett, és csökkentette a különbségeket, de éppen ez volt a cél.
Az SQL lekérdezések nettő futási idejét külön mértem.
Egy-egy SQL-lekérdezésnél a kérés elküldésétől
a teljes eredménytábla beérkezéséig eltelt időt mértem,
ezeket összegeztem.
Itt az esetleges hálózati overhead szintén az eredmény lényeges része.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}
        \begin{axis}[
                title={Integrált tesztek futási idejének összehasonlítása (Micronaut$,$ REST)},
    ...
    \begin{tikzpicture}
        \begin{axis}[
                title={Néhány lekérdezéstípus futási idejének összehasonlítása (logaritmikus)},
                title style={
                    at={(-0.1,1.15)},
                    anchor=north west
                },
            ...
            \legend{MySQL,HoloDB (fapados),HoloDB (default),HoloDB (extra konf.)}
        \end{axis}
    \end{tikzpicture}
    ...
    \begin{minipage}[b]{0.4\textwidth}
        \begin{tikzpicture}
            \begin{axis}[
                ...
                \addplot[fill=cyan] % MySQL
                    coordinates { (Adatméret,2324.21) };
                \addplot[fill=blue] % H2
                    coordinates { (Adatméret,564) };
                \addplot[fill=orange] % HoloDB
                    coordinates { (Adatméret,1.16) };
...
Bizonyítottam, hogy összesített performanciában és memóriahasználatban
a HoloDB valóban lényegesen jobban teljesít, mint a vizsgált alternatívák.
